...
.controller('encuestasController', function($scope, turismoReceptor, $ionicLoading, $ionicPopup, $ionicHistory, $rootScope, $cordovaNetwork, ionicToast, $ionicModal, $q, $http, CONFIG, $ionicActionSheet, $location) {
  ...
  $scope.actionSheet=function (id, last_section, is_offline) {
    ...
    for(let i=0; i<last_section; i++){
      switch(i) {
        case 0:
          buttonsAction.push( {text: '<i class="icon ion ion-information-circled balanced"></i> Información General' });
        break;

        case 1:
          buttonsAction.push( {text: '<i class="icon ion ion-clock balanced"></i> Estancia' });
        break;

        case 2:
          buttonsAction.push( {text: '<i class="icon ion ion-model-s balanced"></i> Transporte' });
        break;

        case 3:
          buttonsAction.push( {text: '<i class="icon ion ion-ios-people balanced"></i> Viaje en grupo' });
        break;

        case 4:
          buttonsAction.push( {text: '<i class="icon ion ion-cash balanced"></i> Gastos de viaje' });
        break;

        case 5:
          buttonsAction.push( {text: '<i class="icon ion ion-person-add balanced"></i> Percepción de viaje' });
        break;

        case 6:
          buttonsAction.push( {text: '<i class="icon ion ion-speakerphone balanced"></i> Como se enteran?' });
        break;
      }
    }
